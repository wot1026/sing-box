...
get_info() {
    ...
    # VLESS 链接格式：encryption=none，不带 TLS（Argo 负责 TLS），sni/host 指向 Argo 域名
    VLESS_LINK="vless://${uuid}@${CFIP}:${CFPORT}?encryption=none&security=tls&sni=${argodomain}&type=ws&host=${argodomain}&path=%2Fvless-argo%3Fed%3D2560&fp=&allowInsecure=false#${node_prefix} argo"
...
# 更新 VLESS Argo 域名到 url.txt
change_argo_domain() {
    # 直接用 sed 替换 url.txt 中 vless 链接的 sni= 和 host= 参数值
    sed -i "s/sni=[^&&#]*/sni=${ArgoDomain}/g; s/host=[^&&#]*/host=${ArgoDomain}/g" "$client_dir"
    local new_vless
    new_vless=$(grep '^vless://' "$client_dir")
    green "VLESS 节点已更新\n"
    purple "${new_vless}\n"
}
...
change_config() {
    ...
    case "$choice" in
        ...
        2)
            ...
            ;;
        3)
            reading "\n请输入新的 VLESS-Argo 端口（直接回车随机生成）: " new_port
            [ -z "$new_port" ] && new_port=$(shuf -i 10000-65000 -n 1)
            jq --argjson port "$new_port" \
               '(.inbounds[] | select(.type == "vless").listen_port) = $port' \
               "$inbounds_file" > "${inbounds_file}.tmp" && mv "${inbounds_file}.tmp" "$inbounds_file"
            ...
            get_info
            green "\nVLESS-Argo 端口已修改为：${purple}${new_port}${re}\n"
            ;;
        4)
            clear
            ...
            # 替换 vless 链接中 @server:port 部分
            sed -i -E "s|vless://([^@]+)@[^?]+\?|vless://\1@${cfip}:${cfport}?|" "$client_dir"
            new_vless=$(grep '^vless://' "$client_dir")
